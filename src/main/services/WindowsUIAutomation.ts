...
try {
  ...
  if ($newMessageButton) {
    ...
    Start-Sleep -Milliseconds 6000  # Wait for contact to load
...
`;
      ...
      return new Promise((resolve) => {
        ...
        psProcess.stdout?.on('data', (data: Buffer) => {
          ...
          log.info('🔄 Phone Link automation:', data.toString().trim());
        });
        ...
        psProcess.on('close', (code) => {
          ...
          if (code === 0 || output.includes('SUCCESS')) {
            log.info('✅ Message sent successfully via single script!');
            resolve(true);
          } else {
            log.error(`❌ Single script failed. Code: ${code}, Error: ${errorOutput}`);
            resolve(false);
          }
        });
